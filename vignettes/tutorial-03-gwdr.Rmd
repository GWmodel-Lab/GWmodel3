...
Let us set the data `LondonHP` as an example, and show how to use `sdr()`.
Assume that the `PURCHASE` is the dependent variable, 
while `FLOORSZ`, `PROF` and `UNEMPLOY` are independent variable, 
we can calibrate a SDR model with the following code.
...
This function returns a object of class name `sdrm`.
...
Like other functions, `sdrm` objects provide a `$SDF` variable 
...
This package provides a S4 class named `SDRConfig`,
...
We can use the function `sdr_config()` to create an object.

```{r}
sdr_config(0.618, TRUE, "bisquare")
```

Replicating via `rep()` is enabled, but only the `times` parameter is supported.

```{r}
rep(sdr_config(36, TRUE, "bisquare"), 2)
```

## Use SDRConfig to configure arguments

The function `sdr()` can not only set a uniform configuration,
but also set each a configuration for dimensions.

### Uniform configuration

To set a uniform configuration for all dimensions, just pass a list of only one `SDRConfig` object.
...
m2 <- sdr(
 formula = PURCHASE ~ FLOORSZ + UNEMPLOY + PROF,
 data = LondonHP,
 config = rep(sdr_config(0.2), times = 2)
)
```
...
we need to input a list of `SDRConfig` objects
...
m3 <- sdr(
 formula = PURCHASE ~ FLOORSZ + UNEMPLOY + PROF,
 data = LondonHP,
 config = list(sdr_config(bw = 0.5, kernel = "bisquare"),
               sdr_config(bw = 0.5, kernel = "bisquare"))
)
m3
```
...
The bandwidth optimization algorithm for SDR requires a set of initial values for each parameter.
...
Apply the `step()` function to `sdrm` objects to select the best model.
...
If it is not set, configurations in the `sdrm` object will be applied.
